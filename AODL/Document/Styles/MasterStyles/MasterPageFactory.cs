...
namespace AODL.Document.Styles.MasterStyles
{
	...
	public class MasterPageFactory
	{
        ...
        public static void MoveMasterStyles(XmlDocument contentDocument, XmlNodeList nodeAutomaticStyles, XmlNodeList nodeMasterPages, XmlNamespaceManager namespaceMng)
        {
            // First import all automatic styles
            XmlNode contentAutomaticStyles = contentDocument.SelectSingleNode(
                "//office:automatic-styles", namespaceMng);
            if (contentAutomaticStyles != null)
            {
                foreach (XmlNode childNode in nodeAutomaticStyles)
                {
                    XmlNode importNode = contentDocument.ImportNode(childNode, true);
                    contentAutomaticStyles.AppendChild(importNode);
                }
            ...
            // Second import master pages
            foreach (XmlNode nodeMasterPage in nodeMasterPages)
            {
                XmlNode importedMasterPages = contentDocument.ImportNode(nodeMasterPage, true);
                contentDocument.LastChild.AppendChild(importedMasterPages);
            }

            // clear nodes in style.xml
            foreach (XmlNode nodeAutomaticStyle in nodeAutomaticStyles)
            {
                nodeAutomaticStyle.RemoveAll();
            }

            // clear master pages in style.xml
            foreach (XmlNode nodeMasterPage in nodeMasterPages)
            {
                nodeMasterPage.RemoveAll();
            }
        }
