...
{
	public class ODFCheckBox: ODFFormControl
	{
		...
		/// </summary>
		public XmlBoolean IsTristate
		{
			get
			{
				XmlNode xn = this._node.SelectSingleNode("@form:is-tristate", 
					this._document.NamespaceManager);
				if (xn == null) return XmlBoolean.NotSet;
			...
			set
			{
				...
				XmlNode nd = this._node.SelectSingleNode("@form:is-tristate", 
					this._document.NamespaceManager);
				if (nd == null)
					nd = this.Node.Attributes.Append(this._document.CreateAttribute("is-tristate", "form"));
				nd.InnerText = s;
			}
		}
		...
		{	
			get
			{
				XmlNode xn = this._node.SelectSingleNode("@form:state", 
					this._document.NamespaceManager);
				if (xn == null) return State.NotSet;

				string s = xn.InnerText;	
				State at;
				switch (s)
				{
					case "unchecked": at = State.Unchecked; break;
					case "checked": at = State.Checked; break;
					case "unknown": at = State.Unknown; break;
					default: at = State.NotSet; break;
				}
				return at;
			}
			set
			{
				...
				switch (value)
				{
					case State.Unchecked: s = "unchecked"; break;
					case State.Checked: s = "checked"; break;
					case State.Unknown: s = "unknown"; break;
					default: return;
				}
				XmlNode nd = this._node.SelectSingleNode("@form:state", 
					this._document.NamespaceManager);
				if (nd == null)
					nd = this._node.Attributes.Append(this._document.CreateAttribute("state", "form"));
				nd.InnerText = s;
			}
		}
		...
		{	
			get
			{
				XmlNode xn = this._node.SelectSingleNode("@form:current-state", 
					this._document.NamespaceManager);
				if (xn == null) return State.NotSet;
			...
			set
			{
				...
				XmlNode nd = this._node.SelectSingleNode("@form:current-state", 
					this._document.NamespaceManager);
				if (nd == null)
					nd = this._node.Attributes.Append(this._document.CreateAttribute("current-state", "form"));
				nd.InnerText = s;
			}
		}
		...
		public ODFCheckBox(ODFForm ParentForm, ContentCollection contentCollection, string id, string x, string y, string width, string height): base (ParentForm, contentCollection, id, x, y, width, height)
		{}

		public ODFCheckBox(ODFForm ParentForm, XmlNode node): base(ParentForm, node)
		{}

		protected override void CreateBasicNode()
		{
			XmlNode xe = this._document.CreateNode("checkbox", "form");
			Node.AppendChild(xe);
			Node = xe;
			this.ControlImplementation = "ooo:com.sun.star.form.component.CheckBox";
		}
	}
}
