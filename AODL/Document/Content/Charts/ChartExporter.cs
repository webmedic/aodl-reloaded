		...
		public void Export(IDocument document,string dir)
		{
			try
			{
				...
				foreach ( EmbedObject eo in document.EmbedObjects)
				{
					...
					{
						this.WriteSingleFiles(((Chart)eo).ChartStyles.Styles,dir+eo.ObjectName + Path.DirectorySeparatorChar +ChartStyles.FileName);
                        this.WriteSingleFiles(((Chart)eo).ChartDoc, dir + eo.ObjectName + Path.DirectorySeparatorChar + "content.xml");
